...
NPDbuffer::NPDbuffer(const char *src, size_t size)
{
    ...
    {
        NPD_THROW("NPDbuffer()使用了错误的参数[%p][%lu]", src, size);
    }
    _buffer.clear();
    _buffer.resize(size);
...
NPDbuffer NPDbuffer::get(uint32_t pos, uint32_t size)
{
    ...
    {
        NPD_THROW("NPDbuffer::get()长度[%u]大于有效长度[%lu]", size, _buffer.size());
    }
...
const char *NPDbuffer::query(uint32_t pos, uint32_t size) const
{
    if(pos + size > _buffer.size())
    {
        NPD_THROW("NPDbuffer::query()长度[%u]大于有效长度[%lu]", size, _buffer.size());
    }
...
void NPDbuffer::set(uint32_t pos, const char *src, uint32_t srcsize)
{
    ...
    {
        NPD_THROW("NPDbuffer::set() 1 源长度[%u]大于当前有效长度[%lu]", pos + srcsize, _buffer.size());
    }
...
void NPDbuffer::set(uint32_t pos, const NPDbuffer &src)
{
    ...
    {
        NPD_THROW("NPDbuffer::set() 2 源长度[%u]大于当前有效长度[%lu]", pos + src.size(), _buffer.size());
    }
...
void NPDbuffer::dump() const
{
    ...
    {
        NPD_THROW("NPDbuffer::dump 1 错误的buffer长度[%lu]", _buffer.size());
    }
}
...
void NPDbuffer::dump(uint32_t pos, uint32_t size) const
{
    if(_buffer.size() > 0)
    {
        ...
        if(pos + size > _buffer.size())
        {
            NPD_THROW("NPDbuffer::dump 2 源长度[%u]大于当前有效长度[%lu]", pos + size, _buffer.size());
        }
    ...
    {
        NPD_THROW("NPDbuffer::dump 2 错误的buffer长度[%lu]", _buffer.size());
    }
}
...
void NPDbuffer::remove(uint32_t pos, uint32_t size)
{
    ...
    if(pos+size > _buffer.size())
    {
        NPD_THROW("NPDbuffer::remove()源长度[%u]大于当前有效长度[%lu]", pos + size, _buffer.size());
    }
