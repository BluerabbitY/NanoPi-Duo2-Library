...
// 读取数据
int NPDSerial::read(void *src, int size, int timeout)
{
...
// 写入数据
int NPDSerial::write(void *dest, int size, int timeout)
{
    FD_IS_READY(_serial_fd);
...
// 读取一字节数据
int NPDSerial::read_8bit(uint8_t &dest, int timeout)
{
    FD_IS_READY(_serial_fd);
...
// 写入一字节数据
int NPDSerial::write_8bit(uint8_t src, int timeout)
{
    FD_IS_READY(_serial_fd);
